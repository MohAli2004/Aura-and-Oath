...
<div class="max-w-3xl mx-auto px-4 sm:px-6 py-16 space-y-6">
    ...
    <div>
        <h2 class="font-display text-2xl">What payment methods do you accept?</h2>
    ...
    <div><h2 class="font-display text-2xl">Can I return an item?</h2><p class="text-taupe mt-2">Only if there is a real problem — a defective or damaged item, a missing item, or a different item than the one you ordered. Used or broken-after-delivery items cannot be returned. Start a return on the <a href="{{ route('returns.index') }}" class="underline">Returns</a> page within {{ \App\Support\TrustMessaging::returnWindowLabel() }} of delivery, with a photo and details. See our <a href="{{ route('pages.show', 'returns-policy') }}" class="underline">returns policy</a>.</p></div>
</div>
@endsection
